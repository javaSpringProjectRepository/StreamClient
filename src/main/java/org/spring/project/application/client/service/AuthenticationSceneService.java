...
//        Scene element events - start
        authenticationScene.getRegistration_scene_button().setOnAction(event -> {
            toRegistrationScene();
        });

        authenticationScene.getAuthentication_scene_button().setOnAction(event -> {
            toAuthenticationScene();
        });
                        ...
                        .subscribe(responseEntity -> {
                            if (responseEntity.getStatusCode().equals(CREATED)) {
                                ...
                                if (responseEntity.getStatusCode().equals(BAD_REQUEST)) {
                                    ...
                                    log.error("Ошибка при попытке регистрации: " + responseEntity.getStatusCode());
                                }
                            }
                        });
            } else {
                errorText.append("Одно из полей не заполнено\n");
            }
            if (!errorText.toString().equals("")) {
                Platform.runLater(() -> {
                    authenticationScene.getAuthentication_error_list().setText(errorText.toString());
                });
            }
        ...
        authenticationScene.getAuthentication_button().setOnAction(event -> {
            ...
            if (!authenticationScene.getAuthentication_login_field().getText().isEmpty() &&
                ...
                requestBuilder.builderRequest(urlProperties.getPostMethod(), urlProperties.getAuthentication(), null, authenticationDto, false)
                        .toBodilessEntity()
                        .onErrorContinue((throwable, o) -> {
                            errorText.append("Не удалось авторизироваться\n");
                            Platform.runLater(() -> authenticationScene.getAuthentication_error_list().setText(errorText.toString()));
                        })
                        .subscribe(responseEntity -> {
                            if (responseEntity.getStatusCode().equals(HttpStatus.OK)) {
                                String accessToken = responseEntity.getHeaders().getFirst(tokenProperties.getAccessTokenKey());
                                String refreshToken = responseEntity.getHeaders().getFirst(tokenProperties.getRefreshTokenKey());
                                if (accessToken != null && accessToken.startsWith(tokenProperties.getAccessTokenPrefix()) &&
                                        refreshToken != null && refreshToken.startsWith(tokenProperties.getRefreshTokenPrefix())) {
                                    ...
                                    try {
                                        ...
                                        errorText.append("Ошибка при создании или записи в файл профиля\n");
                                    }
                                ...
                                if (responseEntity.getStatusCode().equals(UNAUTHORIZED)) {
                                    errorText.append("Неверное имя пользователя или пароль\n");
                                } else {
                                    errorText.append(responseEntity.getStatusCode().toString()).append("\n");
                                }
                            }
                        });
            } else {
...
//        Scene element events - end
    ...
    private void toAuthenticationScene() {
        ...
        authenticationScene.getAuthentication_vbox().getChildren().add(authenticationScene.getAuthentication_vbox().getChildren().indexOf(authenticationScene.getAuthentication_email_field()), authenticationScene.getAuthentication_button());
        authenticationScene.getAuthentication_vbox().getChildren().add(authenticationScene.getAuthentication_vbox().getChildren().indexOf(authenticationScene.getAuthentication_email_field()), authenticationScene.getRegistration_scene_button());
        authenticationScene.getAuthentication_vbox().getChildren().remove(authenticationScene.getAuthentication_email_field());
        authenticationScene.getAuthentication_vbox().getChildren().remove(authenticationScene.getRegistration_button());
        authenticationScene.getAuthentication_vbox().getChildren().remove(authenticationScene.getAuthentication_scene_button());
    }
    ...
    private void toRegistrationScene() {
        ...
        authenticationScene.getAuthentication_password_field().setText("");
        authenticationScene.getAuthentication_error_list().setText("");
        authenticationScene.getAuthentication_vbox().getChildren().add(authenticationScene.getAuthentication_vbox().getChildren().indexOf(authenticationScene.getAuthentication_button()), authenticationScene.getAuthentication_email_field());
        authenticationScene.getAuthentication_vbox().getChildren().add(authenticationScene.getAuthentication_vbox().getChildren().indexOf(authenticationScene.getAuthentication_button()), authenticationScene.getRegistration_button());
        authenticationScene.getAuthentication_vbox().getChildren().add(authenticationScene.getAuthentication_vbox().getChildren().indexOf(authenticationScene.getAuthentication_button()), authenticationScene.getAuthentication_scene_button());
        authenticationScene.getAuthentication_vbox().getChildren().remove(authenticationScene.getAuthentication_button());
        authenticationScene.getAuthentication_vbox().getChildren().remove(authenticationScene.getRegistration_scene_button());
    }
